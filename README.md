My solutions to a number of coding puzzles.
These are mostly in Python with a handful of Golang.

| Puzzle | Details | Code |
| ------ | ------- | ---- |
| [Advent of Code][aoc]              | The original puzzle. Hard. | [Python][c_aoc_py], [Go][c_aoc_go] |
| [Coding Quest][codingquest]        | Simpler problems. Easy.| [Code][c_cq]    |
| [FlipFlop Codes][flipflop]         | Easy to medium, fun puzzles. | [Code][c_flip]  |
| [Codyssi][codyssi]                 | Good range. Medium. | [Code][c_cody]  |
| [Everybody Codes][everybody.codes] | Close to AoC. Hard. | [Code][c_ec]    |
| [i18n Puzzles][i18n]               | Puzzles centering on internationalization issues. | [Code][c_i18n]  |
| [Marches & Gnats][mng]             | Puzzles that need to be solved by writing a state machine. | [Code][c_mng]   |
| [Tom's Data Onion][data-onion]     | A message wrapped in layers of puzzles. Unwrap layers to find the next layer. | [Code][c_onion] |
| [aquaq][aquaq]                     | | [Code][c_aquaq] |
| [Pi Coding Quest][pi-coding]       | | [Code][c_pi]    |

The `lib/` contains a number of helpers and base runner code.

[aoc]: http://adventofcode.com/
[codingquest]: https://codingquest.io/
[codyssi]: https://codyssi.com/
[everybody.codes]: https://everybody.codes/
[i18n]: https://i18n-puzzles.com/
[pi-coding]: https://ivanr3d.com/projects/pi/
[mng]: https://mng.quest/
[aquaq]: https://challenges.aquaq.co.uk/
[flipflop]: https://flipflop.slome.org/
[data-onion]: https://www.tomdalling.com/toms-data-onion/

[c_aoc_py]: http://github.com/isaacG/advent-of-Code/tree/main/advent_of_code/
[c_aoc_go]: http://github.com/isaacG/advent-of-Code/tree/main/advent_of_code.go/
[c_cq]: https://github.com/IsaacG/Advent-of-Code/tree/main/codingquest
[c_cody]: https://github.com/IsaacG/Advent-of-Code/tree/main/codyssi
[c_ec]: https://github.com/IsaacG/Advent-of-Code/tree/main/everybody_codes
[c_i18n]: https://github.com/IsaacG/Advent-of-Code/tree/main/i18n
[c_pi]: https://github.com/IsaacG/Advent-of-Code/tree/main/ivanr3d.pi
[c_mng]: https://github.com/IsaacG/Advent-of-Code/tree/main/mng
[c_aquaq]: https://github.com/IsaacG/Advent-of-Code/tree/main/aquaq
[c_flip]: https://github.com/IsaacG/Advent-of-Code/tree/main/flipflop
[c_onion]: https://github.com/IsaacG/Advent-of-Code/tree/main/other/data_onion
