#!/bin/python
"""Advent of Code: Day 19. Merge disperate sensor maps of beacons into one unified map."""
...
@functools.cache
def potential_match(map_a, map_b) -> bool:
    ...
    pairs = itertools.product(
        relative_offsets(map_a),
        relative_offsets(map_b)
    )
...
def merge(beacon_maps):
    ...
    while to_merge:
        ...
        for fixed, candidate in itertools.product(reversed(merged_maps.keys()), list(to_merge)):
            if not potential_match(candidate, fixed):
                continue

            # Try to fit the loose piece in every possible orientation.
            for orientation in orientations(candidate):
...
def solve(data: frozenset[frozenset[tuple[int, int, int]]], part: int) -> int:
    """Return details about beacon groups."""
    merged_map = merge(data)
    if part == 1:
        merged_beacons = {
    ...
    return max(
        ...
        for a, b in itertools.combinations(merged_map.values(), r=2)
    )


def input_parser(data: str) -> frozenset[frozenset[tuple[int, ...]]]:
...
TESTS = [(1, SAMPLE, 79), (2, SAMPLE, 3621)]
