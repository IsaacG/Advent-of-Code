...
def solve(data: Battle, part: int) -> int:
    """Solve the exercise."""
    boost = 0 if part == 1 else get_boost(data)
    return data.battle(boost)[1]


def get_boost(data: Battle) -> int:
    """Return the minimum boost needed for the immune system to win."""
    # Find an upper limit using exponential growth.
    upper_limit = 2
    while not data.battle(upper_limit)[0]:
        upper_limit *= 2
...
def input_parser(data: str) -> Battle:
    """Parse the input."""
    groups = []
    num = 1
...
TESTS = [(1, SAMPLE, 5216), (2, SAMPLE, 51)]
