#!/usr/bin/env python
"""Day 20. Process satellite images. Stitch image tiles and find sea monsters in them."""
...
SAMPLE = input_data.D20
MONSTER = input_data.D20_MONSTER
...
class Tile:
    ...
    def pair(self, other):
        """Pair up two tiles."""
        _, matching_side = self._common_edge(other)
...
def stitched_tiles(blocks: list[str]) -> dict[int, Tile]:
...
def find_corner(tiles: dict[int, 'Tile'], dirs: list[Side]) -> int:
...
def stitched_image(tiles: dict[int, Tile]) -> Image:
    ...
    row_left = tiles[find_corner(tiles, (Side['TOP'], Side['LEFT']))]
    while row_left:
        ...
        for i in range(4):
            ...
            for origin_y in range(len(image) - len(MONSTER) + 1):
                for origin_x in range(len(image[0]) - len(MONSTER[0]) + 1):
                    ...
                    found = True
                    for y, monsters in enumerate(MONSTER):
                        for x, monster in enumerate(monsters):
                            if monster == '#' and image[y + origin_y][x + origin_x] != '#':
                                found = False
                                break
                        if not found:
                            break
...
def part1(blocks: list[str], testing: bool) -> int:
