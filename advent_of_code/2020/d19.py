...
import re
import data as test_inputs

TEXT_RE = re.compile(r'"(.*)"')


def generate_regexes(raw_rules: dict[int, str]) -> dict[int, re.Pattern]:
...
def part2(data: tuple[dict[int, str], list[str]]) -> int:
    ...
    for i in (0, 8, 11):
        if i in raw_rules:
            del raw_rules[i]
    regexps = generate_regexes(raw_rules)
...
def part1(data: tuple[dict[int, str], list[str]]) -> int:
    """Find the num of lines that match the regex-like rules."""
    raw_rules, inputs = data
    # Map input to regexps.
    regexps = generate_regexes(raw_rules).values()
