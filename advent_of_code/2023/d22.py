...
def solve(data: dict[int, tuple[int, ...]], part: int) -> int:
    ...
    settled = set[int]()
    settled_bricks = dict[int, tuple[int, ...]]()
...
def input_parser(data: str) -> dict[int, tuple[int, ...]]:
    """Parse the input data."""
    nums = aoc.parse_re_findall_int(aoc.RE_INT).parse(data)
    return {idx: tuple(values) for idx, values in enumerate(nums)}
