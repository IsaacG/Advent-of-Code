...
def solve(data: list[list[int]], part: int) -> int:
    ...
    """
    ranges: dict[int, int] = dict(sorted(data.items()))  # type: ignore
...
def part_two_groupwise(intervals: dict[int, int]) -> int:
    ...
    Note: a scanner with period 4 and depth 1 is functionally equivalent
    to a scanner with period 4 and depth (1 + 4 * k).
    When computing the depths for a scanner, we can use `depth % interval` to simplify.

    If we have a group of scanners which all have a period of 4,
    we can denote all soltutions as a subset of `{0, 1, 2, 3} + 4 * k`.
    ...
    Considering only the first interval (k = 1) that gives
    invalid start delays of `4 - depth` or `period - depth`.
    ...
    If two groups have solutions `{1, 2, 3} + 5 * k` and `{2, 3, 4} + 5 * k`
    then the combined solution is {2, 3} + 5 * k`.
    ...
    Instead I expand the solution with the larger period (fewer values generates)
    then check if those would be in the other solution.
    ...
    If we can process most scanners using the progressive groupwise solution,
    we are left with a single `delay + interval * k`
    ...
    remaining_intervals = [
        (depth, interval) for depth, interval in intervals.items()
